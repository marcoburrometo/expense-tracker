  ...
  return (
    <>
      <div className="glass-fab">
        <button type="button" className="glass-button glass-button--primary pressable" aria-label="Aggiungi veloce" onClick={()=> setOpen(true)}>＋</button>
      </div>
      {open && (
        <div className="modal-overlay">
          <dialog open className="modal-panel glass-panel modal-enter p-5 space-y-4 bg-transparent">
            <div className="flex items-start justify-between">
              <h2 className="font-semibold text-lg">Aggiungi Rapido</h2>
              <button onClick={()=> setOpen(false)} className="glass-button glass-button--neutral glass-button--sm" aria-label="Chiudi">✕</button>
            </div>
            <form onSubmit={submit} className="space-y-3 text-sm">
              <input className="glass-input w-full" placeholder="Descrizione" value={description} onChange={e=>setDescription(e.target.value)} autoFocus />
              <div className="flex gap-2">
                <input type="number" step="0.01" className="glass-input w-full" placeholder="Importo" value={amount} onChange={e=>setAmount(e.target.value)} />
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={()=>setOpen(false)} className="glass-button">Annulla</button>
                <button type="submit" disabled={saving} className="glass-button glass-button--success">{saving?'...':'Salva'}</button>
              </div>
            </form>
          </dialog>
        </div>
