      ...
      <div className="relative w-full overflow-hidden">
        <svg width={width} height={height} className="block">
          ...
          {yTicks.map(v=> {
            const y = scaleY(v);
            return <g key={v}>
              <line x1={0} y1={y} x2={width} y2={y} stroke={v===0? '#64748b':'#cbd5e1'} strokeDasharray={v===0? '':'4 4'} strokeWidth={v===0?1.2:1} />
              <text x={4} y={y-2} fontSize={10} fill="#475569">€ {v.toFixed(0)}</text>
            </g>;
          })}
          {/* Area + Line */}
          {areaPath && <path d={areaPath} fill="url(#areaGrad)" stroke="none" />}
          {pathActual && <path d={pathActual} fill="none" stroke="#2563eb" strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />}
          {trendLine && <path d={trendLine} fill="none" stroke="#ec4899" strokeWidth={2} strokeDasharray="5 4" />}
          {/* Today line if in range */}
          {(() => { const todayStr = new Date().toISOString().slice(0,10); const idx = points.findIndex(p=>p.date===todayStr); if (idx>-1) { const x = scaleX(idx); return <line x1={x} y1={0} x2={x} y2={height} stroke="#f59e0b" strokeDasharray="4 3" />; } return null; })()}
          {/* Month ticks */}
          {months.map(m=> {
            const x = scaleX(m.idx);
            return <g key={m.label}>
              <line x1={x} y1={height} x2={x} y2={height-6} stroke="#475569" />
              <text x={x+2} y={height-6} fontSize={9} fill="#475569" transform={`rotate(-45 ${x+2},${height-6})`}>{m.label}</text>
            </g>;
          })}
          {/* Hover marker */}
          {hoverPoint && (()=> { const i = hoverIdx!; const x = scaleX(i); const y = scaleY(hoverPoint.balance); return <g pointerEvents="none"> <circle cx={x} cy={y} r={4} fill="#1d4ed8" stroke="#fff" strokeWidth={2} /> <line x1={x} y1={0} x2={x} y2={height} stroke="#94a3b8" strokeDasharray="3 3" /> </g>; })()}
          <rect x={0} y={0} width={width} height={height} fill="transparent" onMouseMove={onMove} onMouseLeave={onLeave} />
        </svg>
        {hoverPoint && <div className="absolute pointer-events-none text-xs bg-white dark:bg-neutral-700 border border-neutral-300 dark:border-neutral-600 rounded shadow px-2 py-1" style={{ left: `${scaleX(hoverIdx!)}px`, top: 4, transform: 'translateX(-50%)' }}>
          <div className="font-mono">{hoverPoint.date}</div>
          <div className="font-semibold">€ {hoverPoint.balance.toFixed(2)}</div>
