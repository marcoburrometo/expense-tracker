...
export const DayCell: React.FC<DayCellProps> = ({ date, bucket, onOpenDay, fmt }) => {
  if(!date){
    return <div className="opacity-0 h-24" aria-hidden />; // spacer
  }
  const key = fmt(date);
  ...
  return (
    <button
      ...
      style={style}
      className={`relative flex flex-col text-left h-24 p-1 rounded-lg glass-panel glass-panel--subtle overflow-hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/70 group transition-all duration-150 hover:shadow-[0_4px_14px_-2px_rgba(0,0,0,0.35)] ${isToday ? 'ring-2 ring-accent/70 shadow-[0_0_0_2px_rgba(99,102,241,0.4)]' : ''} ${bgExtra}`}
  ...
  <div className="flex flex-col gap-0.5 overflow-hidden relative flex-1 pr-0.5">
        <div className="overflow-y-auto glass-scroll pr-0.5">
            ...
            return (
              <div key={e.id} className="text-[9px] truncate flex justify-between gap-1">
                <span className="flex-1 truncate">
                  {e.description}
                  {synthetic && <span className="ml-0.5 inline-block px-1 rounded bg-indigo-500/55 dark:bg-indigo-400/40 text-[7px] leading-none text-indigo-900 dark:text-indigo-200">S</span>}
                </span>
