...
export const DashboardStats: React.FC = () => {
  ...
  return (
    <div className="grid gap-3 sm:gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
      <StatCard label={t('stats.incomeTotal')} value={`€ ${totalIn.toFixed(2)}`} accent="green" hint={t('stats.incomeTotal') + ' · ' + 'Totale'} />
      <StatCard label={t('stats.expenseTotal')} value={`€ ${totalOut.toFixed(2)}`} accent="red" hint={t('stats.expenseTotal') + ' · ' + 'Totale'} />
